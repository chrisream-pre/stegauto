...
def encrypt_password(password, key):
    # Encrypt the password using aes256 encryption
...
def decrypt_password(encrypted_password, key):
    # Decrypt the password using aes256 encryption
...
def embed_password_in_image(image_path, password, key, output_path):
    ...
    try:
        ...
        print(f"Error embedding password into image: {e}")
        raise
...
def extract_password_from_image(image_path, key):
    ...
    try:
        ...
        print(f"Error extracting password from image: {e}")
        raise
...
def main():
    ...
    if args.mode == 'embed':
        if not args.password or not args.output_image:
            print("Embedding mode requires both password and output image path.")
            return
